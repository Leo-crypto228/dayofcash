...
export default function Hilo({ game }) {
  ...
  const guess = (dir) => {
    ...
    const next = hiloDraw(deckRef.current, cur, dir, balance)
    if (!next) return
      ...
      <div className="hilo-wrap">
        ...
        <div className="s-grid2">
          <div className="s-field">
            ...
            <div className="s-box"><span style={{ fontWeight: 600 }}>{fmt(Math.max(0, capProfit(bet * mult * stepHigher - bet, balance)))}</span></div>
          </div>
          <div className="s-field">
            <label>Profit Inférieur ({stepLower.toFixed(2)}×)</label>
            <div className="s-box"><span style={{ fontWeight: 600 }}>{fmt(Math.max(0, capProfit(bet * mult * stepLower - bet, balance)))}</span></div>
          </div>
        </div>
